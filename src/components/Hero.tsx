  ...
  return (
    ...
    >
      <motion.div
        ...
        className="px-3.5 py-2 rounded-xl border border-border/60 bg-card/80 backdrop-blur-md text-xs font-medium text-foreground/80 shadow-lg whitespace-nowrap"
      >
        {label}
      </motion.div>
    </motion.div>
  ...
  return (
    <div className="relative mx-auto w-full max-w-sm" style={{ perspective: 1200 }}>
      ...
      <motion.div
        ...
        className="relative z-10 rounded-3xl border border-border/60 bg-card/70 backdrop-blur-2xl p-8 sm:p-10 shadow-2xl"
      >
        <div className="relative flex items-center gap-2 mb-8">
          ...
          <span className="text-[11px] uppercase tracking-widest text-muted-foreground font-medium">Live results</span>
        </div>
        ...
        <div className="relative h-28 flex flex-col justify-center">
          <AnimatePresence mode="wait">
            ...
            >
              <p className="text-5xl sm:text-6xl font-display font-bold text-foreground tracking-tight">{stat.value}</p>
              <p className="mt-2 text-sm text-muted-foreground max-w-[220px]">{stat.label}</p>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="relative mt-8 flex items-center gap-1.5">
          {proofStats.map((s, i) => (
            <span key={s.value} className={`h-1 rounded-full transition-all duration-500 ${i === statIndex ? "w-6 bg-primary" : "w-1.5 bg-muted"}`} />
          ))}
        </div>
      </motion.div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 min-h-screen grid lg:grid-cols-2 gap-16 items-center px-6 sm:px-10 lg:px-16 max-w-7xl mx-auto py-32 lg:py-20">
        <div className="text-left">
          <motion.span
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-border/60 bg-card/80 backdrop-blur-md text-xs font-medium text-muted-foreground mb-6"
          >
      ...
      <motion.a
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors"
